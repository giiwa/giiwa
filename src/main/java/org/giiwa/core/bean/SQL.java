...
public class SQL {
   ...
   * @param h
   * @param sql
   *          "select * from <table> where a=1 orderby b <desc> offset 1 limit 10";
   * @return
   */
  public static List<Bean> query(DBHelper h, String sql) {
    ...
    try {
      ...
      Beans<Bean> bs = h.load(q.getString("tablename"), q1, q.getInt("offset", 0), q.getInt("limit", 10), Bean.class,
          X.EMPTY);
  ...
  public static Bean get(DBHelper h, String sql) throws SQLException {
    ...
    try {
      ...
      return h.load(q.getString("tablename"), q1, Bean.class, X.EMPTY);
    } catch (Exception e) {
      log.error(sql, e);
    }
    return null;
