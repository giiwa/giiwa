...
class Logger {
	...
	public static void log(long seq, String direction, String to, MQ.Request r) {
		if (islog) {
			try {
				JSON p = JSON.create();
				p.put("node", MQ._node);
	...
	class StatusTask extends Task {
		...
		public void onExecute() {
			...
			String s = p.toString();
			MQ.log(JSON.create().append("message", s).append("time1", System.currentTimeMillis())
					.append("node", MQ._node).append("len", s.length()).append("flag", flag).append("type", 0)
					.append("seq", seq.incrementAndGet()));
		}
