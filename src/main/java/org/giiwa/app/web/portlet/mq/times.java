...
public class times extends portlet {
	...
	public void get() {
		...
		W q = W.create().and(X.NODE, id).and(X.NAME, "read")
		...
		if (bs != null && !bs.isEmpty()) {
			...
			Beans<_MQ.Record> list2 = _MQ.Record.dao.load(W.create().and(X.NODE, Local.id()).and(X.NAME, "write")
	...
	public void data() {
		...
		Beans<_MQ.Record> bs = _MQ.Record.dao.load(W.create().and(X.NODE, id).and(X.NAME, "read")
				.and(X.CREATED, Global.now() - X.AHOUR, W.OP.gte).sort(X.CREATED, -1), 0, 60);
		if (bs != null && !bs.isEmpty()) {
			Collections.reverse(bs);
			...
			JSON p = JSON.create();
			p.append(X.NAME, lang.get("mq.read.times")).append("color", "#0a5ea0");
			...
			bs = _MQ.Record.dao.load(W.create().and(X.NODE, Local.id()).and(X.NAME, "write")
					.and(X.CREATED, Global.now() - X.AHOUR, W.OP.gte).sort(X.CREATED, -1), 0, 60);
			if (bs != null && !bs.isEmpty()) {
				Collections.reverse(bs);
				p = JSON.create();
				p.append(X.NAME, lang.get("mq.write.times")).append("color", "#0dad76");
