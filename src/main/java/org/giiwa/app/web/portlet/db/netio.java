...
public class netio extends portlet {
	/**
	 * 
	 */
	...
	public void get() {
		...
		this.show("/portlet/db/netio.html");
	}
	...
	public void more() {
		...
		Beans<_DB.Record> bs = _DB.Record.dao.load(
				W.create().and(X.NODE, Global.id()).and(X.NAME, "status").and(X.CREATED, time, W.OP.gte).sort(X.CREATED, -1), 0,
				24 * 60 * 2);
		if (bs != null && !bs.isEmpty()) {
			Collections.reverse(bs);
			this.set(X.LIST, bs);
		...
		this.show("/portlet/db/netio.more.html");
