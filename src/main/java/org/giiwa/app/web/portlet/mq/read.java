...
public class read extends portlet {
	...
	public void get() {
		...
		long max = X.toLong(1.1 * X.toLong(_MQ.Record.dao.max("max",
				W.create().and(X.NAME, "read").and(X.CREATED, Global.now() - X.AHOUR, W.OP.gte))));
		this.set("max", max);

		this.show("/portlet/mq/read.html");
	}
	...
	public void data() {
		...
		if (bs != null && !bs.isEmpty()) {
			...
			for (String[] s : new String[][] { { "max", "#860606" }, { "avg", "#0dad76" }, { "min", "#0a5ea0" } }) {
				JSON p = JSON.create();
				p.append(X.NAME, lang.get("mq.read." + s[0])).append("color", s[1]);
