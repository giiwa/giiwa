...
public class tcpclosewait extends portlet {
	...
	public void get() {
		...
		long max = X.toLong(1.1 * Stat.max("n14", "node.load", Stat.TYPE.snapshot, Stat.SIZE.min,
				W.create().and("time", System.currentTimeMillis() - X.AHOUR, W.OP.gte)));

		this.set("max", max);
		this.show("/portlet/node/tcpclosewait.html");
	}
	...
	public void data() {
		...
		if (bs != null && !bs.isEmpty()) {
			...
			long max = Stat.max("n14", "node.load", Stat.TYPE.snapshot, Stat.SIZE.min,
					W.create().and("time", System.currentTimeMillis() - X.AHOUR, W.OP.gte));
