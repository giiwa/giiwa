...
public class user extends Model {
   ...
   */
	...
	public void captcha() {
		String text = null;
		text = (String) this.getSession().get("captcha.text");

		String c = module.get("captcha.text", "digital");
		int len = Bean.toInt(module.get("captcha.length", "5"));
		if (len < 1) {
			len = 5;
		}
		if ("digital".equals(c)) {
			text = UID.digital(len);
		} else {
			text = UID.random(len);
		}
		this.getSession().set("captcha.text", text).store();
		...
		try {
			...
			BufferedImage img = null;// creatImage(text);
			...
			ImageIO.write(img, "png", this.getOutputStream());
   ...
   * Captcha_validate.
   */
	@Path(path = "captcha/validate")
	public void captcha_validate() {

		JSONObject jo = new JSONObject();
		String text = (String) this.getSession().get("captcha.text");
		String t = this.getString("text");
		if (t != null && t.equalsIgnoreCase(text)) {
			jo.put(X.STATE, 200);
			jo.put(X.MESSAGE, "yes");
		} else {
			jo.put(X.STATE, 201);
			jo.put(X.MESSAGE, "no");
		}
	...
	public void register() {
		if (!"true".equals(ConfigGlobal.s("user.register", "true"))) {
	...
	public boolean go() {
		...
		if (s.has("oauth.callback")) {
			...
			try {
				...
				String data = URLEncoder.encode(Base64.encode(DES.encode(jo.toString().getBytes(), key.getBytes())),
						"UTF-8");
	...
	public void login() {
		...
		if (method.isPost()) {
			...
			if (me != null) {
				...
				OpLog.warn(User.class, "user.login",
						lang.get("login.failed") + ":" + name + ", ip:" + this.getRemoteHost(), null);
		 ...
		 */
		if ("true".equals(ConfigGlobal.s("oauth.enabled", null))) {
			String oauth = ConfigGlobal.s("oauth.url", null);
			String appkey = ConfigGlobal.s("oauth.appkey", null);
			String key = ConfigGlobal.s("oauth.key", null);
			if (oauth != null && appkey != null && key != null) {
				String callback = ConfigGlobal.s("oauth.callback", "");
				try {
					...
					url.append(appkey).append("/login").append("?data=").append(data);
	...
	public void verify() {
		...
		if ("name".equals(name)) {
			if (User.exists(new BasicDBObject("name", value))) {
				...
				String allow = ConfigGlobal.s("user.name", "^[a-zA-Z0-9]{4,16}$");
			...
			if (X.isEmpty(value)) {
				...
				String allow = ConfigGlobal.s("user.password", "^[a-zA-Z0-9]{6,16}$");
				...
				if ("login".equals(method)) {
					if (jo.has("uid") && jo.has("time")
						...
						if (me != null) {
							...
							if ("true".equals(ConfigGlobal.s("cross.context", X.EMPTY))) {
								String sessionkey = ConfigGlobal.s("session.key", "user");
								JSONObject j1 = new JSONObject();
								me.toJSON(j1);
							...
							String oauth = ConfigGlobal.s("oauth.url", null);
							String appkey = ConfigGlobal.s("oauth.appkey", null);
							key = ConfigGlobal.s("oauth.key", null);
							if (oauth != null && appkey != null && key != null) {
								String callback = ConfigGlobal.s("oauth.callback", "");
								try {
									StringBuilder url = new StringBuilder(oauth);
			...
			while (bs != null && bs.getList() != null && bs.getList().size() > 0) {
				for (User u : bs.getList()) {
					if (!u.isDeleted()) {
						...
						u.update(V.create("reset_token", token).set("token_expired",
								System.currentTimeMillis() + X.ADAY));
