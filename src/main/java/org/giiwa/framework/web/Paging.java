...
public class Paging {
	 ...
	 */
	public static ArrayList<PageLink> pages(QueryString url, int total,
			int curr, int numofPage) {
		if (numofPage <= 0)
			return null;
		...
		if (curr > 1) {
			list.add(new PageLink(-2, "&lt;", url.set("s",
					((curr - 2) * numofPage)).toString()));
		}
		...
		if (curr < pages) {
			list.add(new PageLink(pages + 10, "&gt;", url.set("s",
					curr * numofPage).toString()));
		}
		...
		for (int i = start; i <= end; i++) {
			if (i == curr)
				continue;
			list.add(new PageLink(i, Integer.toString(i), url.set("s",
					(i - 1) * numofPage).toString()));
		}
	 ...
	 * @return the array list
	 */
	public static ArrayList<PageLink> paging(QueryString url, int total,
			int curr, int numofPage) {
		...
		if (curr < (total / numofPage + 1)) {
			list.add(new PageLink(-1, "&gt;", url.set("s", curr * numofPage)
					.toString()));
		}
		...
		p = new PageLink(n + 1, Integer.toString(n + 1), url.set("s",
				n * numofPage).toString());
		if (!list.contains(p)) {
			list.add(p);
		}
	...
	public static List<PageLabel> create(int total, int s, int ITEM_NUMBER,
		...
		for (int i = 0; i < pages && prev >= 0; i++) {
			list.add(new PageLabel(Integer.toString(prev / ITEM_NUMBER + 1),
					prev, ITEM_NUMBER, prev));
			prev -= ITEM_NUMBER;
		}

		int next = s + ITEM_NUMBER;
		while (list.size() < 2 * pages && next < total) {
			list.add(new PageLabel(Integer.toString(next / ITEM_NUMBER + 1),
					next, ITEM_NUMBER, next));
			next += ITEM_NUMBER;
		}
		...
		if (s > 0) {
			list.add(new PageLabel("&lt;", s - ITEM_NUMBER, ITEM_NUMBER,
					Integer.MIN_VALUE + 1));
		}

		if (prev >= 0) {
			list.add(new PageLabel("&lt;&lt;", 0, ITEM_NUMBER,
					Integer.MIN_VALUE));
		}

		list.add(new PageLabel(Integer.toString(s / ITEM_NUMBER + 1), s,
				ITEM_NUMBER, s, true));
		...
		if (s < total - ITEM_NUMBER) {
			...
			if (next < total) {
				list.add(new PageLabel("&gt;&gt;", total / ITEM_NUMBER
						* ITEM_NUMBER, ITEM_NUMBER, Integer.MAX_VALUE));
			}
		}
