...
giiwa.extend({
	...
	hook: function() {
		...
		function(i, e) {
			...
			if (target == undefined && href != undefined && (href.indexOf('javascript') == -1) && (href.indexOf('#') != 0)) {
		 ...
		 */
		$('#panel select[parentnode=true]').change(
		...
		$('#panel input').bind('focus', function() {
			...
			if (that.attr('max')) {
				// check max
				var value = that.val();
				console.log(value + ", " + value.length + ", " + that.attr('max'));
			}
		});
